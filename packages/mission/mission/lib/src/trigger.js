...
function parseCommand(rawInput) {
    ...
    if (control.length === 0 || control === 'status')
        return { kind: 'status' };
    if (control === 'cancel')
        return { kind: 'cancel' };
...
function executeCommand(ctx, agent, rawInput) {
    ...
    try {
        ...
        switch (cmd.kind) {
            ...
            case 'cancel': {
                if (current === undefined)
                    return { kind: 'success', text: 'No mission to cancel.' };
        ...
        if (error instanceof MissionError)
            return { kind: 'error', text: `Mission command is not valid: ${error.message}` };
        throw error;
    }
}
/**
