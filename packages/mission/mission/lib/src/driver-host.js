...
import { deterministicVerifier, driveMission } from "./driver.js";
import { registerMissionEventTypes } from "./domain.js";
import { subagentExecutor } from "./executor.js";
import { llmDecider, llmReplanner } from "./llm.js";
...
export function apply(ctx, config = {}) {
    ...
    function buildOptions(agent) {
        ...
        if (config.replan === true) {
            ...
            if (config.maxReplans !== undefined)
                options.maxReplans = config.maxReplans;
        }
        return options;
    }
    ...
    function drive(agent) {
        ...
        try {
            ...
            if (view !== undefined && view.tasks.some((t) => t.stored === 'CLAIMED' && t.lease !== null && t.lease.expiresAt <= Date.now()
            ...
            const id = setInterval(() => {
                ...
                for (const agent of agents) {
                    ...
                    try {
                        ...
                        if (view === undefined)
                            continue;
                        const needsReclaim = view.tasks.some((t) => t.stored === 'CLAIMED' && t.lease !== null && t.lease.expiresAt <= Date.now()
                            && t.report === null && !executing.has(t.id));
                        if (needsReclaim)
                            drive(agent);
                    }
