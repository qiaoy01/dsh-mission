...
import { matchesSpec } from "./fold.js";
/** Whether the dependency edge set is acyclic and references only known tasks. */
export function isAcyclic(tasks, deps) {
    const ids = new Set(tasks);
    const adj = new Map();
    for (const t of tasks)
        adj.set(t, []);
    for (const d of deps) {
        if (!ids.has(d.taskId) || !ids.has(d.dependsOn))
    ...
    for (const t of tasks) {
        if ((color.get(t) ?? WHITE) === WHITE && !visit(t))
            return false;
    }
    return true;
}
 ...
 */
export function validateEvent(world, event, now) {
    const v = [];
    ...
    switch (event.type) {
        case 'mission/created': {
            ...
            const active = [...world.values()].some((existing) => existing.status !== 'COMPLETED' && existing.status !== 'FAILED' && existing.status !== 'CANCELLED');
            if (active)
                v.push('an active mission already exists in this session');
            else if (event.objective.trim() === '')
                v.push('objective must be non-empty');
            break;
        }
        case 'mission/plan-committed': {
            ...
            if (new Set(taskIds).size !== taskIds.length)
                v.push('duplicate task ids');
            if (!isAcyclic(taskIds, event.dependencies))
                v.push('dependency graph is not acyclic or has dangling references');
        ...
        case 'mission/task-claimed': {
            ...
            if (event.lease.expiresAt <= now)
                v.push('lease must not already be expired');
            if (t.stored === 'CLAIMED') {
        ...
        case 'mission/task-reported': {
            ...
            if (!t.lease || t.lease.token !== event.token)
                v.push('lease token mismatch: only current holder may report');
            break;
        }
        case 'mission/task-verified': {
            ...
            if (!t.report)
                v.push('verification requires a prior task report (worker cannot self-certify)');
