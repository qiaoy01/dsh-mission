...
import { MissionError } from "./domain.js";
/** One-shot structured call: ask the model to emit one JSON value via a tool call. */
async function structuredCall(ctx, opts) {
    const selection = ctx.get('agentDefaultModel')?.currentSelection?.();
    const provider = opts.provider ?? selection?.provider;
    const model = opts.model ?? selection?.model;
    if (provider === undefined || model === undefined) {
        throw new MissionError(`no model route for ${opts.purpose}: pass provider/model or mount agent-default-model`, 'MISSION_LLM_ROUTE_MISSING');
    }
    ...
    for (const chunk of chunks) {
        ...
        if (block.type === 'tool-call' && block.name === 'respond') {
            try {
                return JSON.parse(block.arguments);
            }
            catch {
                return undefined;
            }
 ...
 * loop commits.
 */
export function llmDecider(ctx, options = {}) {
 ...
 */
export function llmReplanner(ctx, options = {}) {
    return async (view) => {
 ...
 */
export function userApprovalGate(ctx, toolName = 'mission_replan') {
