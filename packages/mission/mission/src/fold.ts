...
export function matchesSpec(t: TaskState, spec: TaskSpec): boolean {
  return t.name === spec.name && t.workerType === spec.workerType && t.priority === spec.priority
}

function getOrCreate(world: MissionWorld, id: string): MissionState {
  let m = world.get(id)
  if (!m) {
    m = newMission(id)
    world.set(id, m)
...
export function effectiveTaskStatus(
  stored: TaskStored,
  deps: string[],
  storedOf: (id: string) => TaskStored | undefined,
): TaskStatus {
 ...
 */
export function missionStatus(m: MissionState): MissionStatus {
  if (m.cancelled) return 'CANCELLED'
  if (m.revision === 0) return 'CREATED'
  ...
  if (failed && !forward) return 'FAILED'
  return 'RUNNING'
}

/** Recompute effective task statuses, then the mission status, for one mission. */
function derive(m: MissionState): void {
  const storedOf = (id: string): TaskStored | undefined => m.tasks.get(id)?.stored
  const depsOf = (id: string): string[] => m.dependencies.filter((d) => d.taskId === id).map((d) => d.dependsOn)
 ...
 */
export function foldMission(events: MissionEvent[]): MissionWorld {
  const world: MissionWorld = new Map()
  for (const ev of events) {
    ...
    switch (ev.type) {
      ...
      case 'mission/task-reported': {
        const t = m.tasks.get(ev.taskId)
        if (t) t.report = { exitCode: ev.exitCode, output: ev.output }
        break
      }
  ...
  for (const m of world.values()) derive(m)
  return world
}
